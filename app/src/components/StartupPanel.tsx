        ...
        <div className="min-h-0 flex-1 overflow-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              {items.map((i) => {
                ...
                return (
                  <tr key={i.id} className="border-b border-line transition-colors duration-150 hover:bg-surface-raised/50">
                    <td className="px-3 py-2 text-content-primary">{i.name}</td>
                    <td className="px-3 py-2"><Badge tone={src.tone}>{src.text}</Badge></td>
                    <td className="max-w-[280px] truncate px-3 py-2 font-mono text-xs text-content-muted" title={i.command}>{i.command}</td>
                    <td className={`px-3 py-2 text-xs ${i.enabled ? 'text-content-secondary' : 'text-content-muted'}`}>
                      {i.enabled ? '启用' : '已禁用'}
                    </td>
                    <td className="px-3 py-2 text-right">
                      {readOnly ? (
                        <span className="text-xs text-content-muted" title="系统级启动项需要管理员权限，v1 只读">只读</span>
                      ) : (
                        <Button
                          variant="secondary"
                          size="xs"
              ...
              {items.length === 0 && (
                <tr><td colSpan={5} className="px-3 py-8 text-center text-content-muted">未发现启动项</td></tr>
              )}
            </tbody>
          </table>
        </div>
