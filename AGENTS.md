...
- **v2.4**（未发版）：桌面工作台（Apple × Codex Desktop Workbench）Phase 1-6。Phase 1 workspace shell（侧栏+顶栏+panelCatalog）+ 设计系统（语义令牌 surface/content/line/focus/success、lucide facade、UI 原语 Button/IconButton/Badge/StateView/PanelActionBar/PanelAlert、kindStyles）+ 布局引擎（openPanel 幂等/preset/persist/活跃面板协调）。Phase 2 六面板 chrome 统一（去 h1→PanelActionBar、emoji→IconButton+lucide、硬编码色→语义令牌/Badge、ProcessTable colSpan 8→9、PluginPanel bg-base-panel→bg-surface-panel）。Phase 3 响应式 tile（useContainerWidth hook + container query，ProcessPanel 侧栏按 tile 宽度≥720px 而非 matchMedia；Snapshot <480px 切换紧凑顶部控制条；PanelActionBar 窄 tile 自动换行）。Phase 4 portal 浮层（Dialog 组件 createPortal+focus trap+Esc+焦点恢复，ConfirmDialog/DiagnosticPreview/RunProfileEditor 迁移，ContextMenu 加 portal）。Phase 5 桌面交互收口（侧栏紧凑左对齐、完整 flex 滚动链、表格初始 Tab 入口、进程详情跨面板聚焦、Snapshot ARIA tabs/ConfirmDialog/reusable StateView、preload 缺失防白屏）。Phase 6 聚焦工作区（最多 3 个可见面板；第二个保持 70/30 主次布局、第三个在活跃 tile 下方 50/50 堆叠，第 4 个替换活跃 tile；顶栏面板计数与「只保留当前面板」；persist v2 将旧拥挤布局收敛到前三个叶子）。Run Profiles 可靠性修复（UX-05/UX-06：spawn 失败置 failed 终态 + 失败徽章；run:getStates 全量同步通道，面板挂载时先订阅→拉快照→重放在途事件，杜绝重复启动同一服务）。操作反馈与危险操作确认（UX-01/03/07/17：kill 确认框列出目标进程清单；操作反馈统一到 toast（warning kind 新增；ToastHost 单实例挂载）；Run Profiles 删除/停止失败有反馈）。审查迭代第三批（UX-12/16/18/24/31：failed run 残留清理、Session 扫描空态、进程→端口联动、快照定位自动开面板、暂停时重试文案、布局持久化同版本校验）。kill 失败原因枚举（UX-02/04/09/16/22/23：killByPids/killProcess 返回逐 pid 状态 killed/protected/denied/not-found，UI 按原因反馈不再三合一；3 面板替换 toast 提示；Run Profiles 列表失败横幅；侧栏复制反馈；单杀文案统一）。项目分组视图虚拟化（UX-13：>100 行虚拟滚动 + GroupRow memo 击穿修复——行组件原始类型 props + store selector 按行订阅，键盘导航 scrollToIndex 兜底）。视觉打磨轨道合并（Toast 通知系统取代全部 alert；Aurora token 迁移清零；Button 原语统一；设计系统 3 级圆角/间距/阴影统一；PerfPanel 玻璃升级 + LoadState→StateView——UX-31 文案随迁自然消除）。审查迭代收官批（UX-08/10/11/14/19/20/21/22/25/26/27/28/29/30：首启欢迎条、窄 tile 详情对话框、预设覆盖确认、CPU 排序冻结、仅监听/全部连接切换、冲突对方 tooltip、半选态、复制失败反馈、端口表焦点钳制、轮询世代计数、错误横幅保留窗口、PerfPanel 错误细节、GPU 列提示、侧栏图标文案）。开发体验增强包（dev-experience-pack 8 子项目：跳转动作闭环/Toast 通知系统/RunProfile 日志闭环/服务守望/数据导出/环境变量对比/启动项管理/一致性收尾——新增 IPC `shell:openTarget`/`shell:openExternalUrl`/`run:getLogs`/`config:exportDataFile`/`startup:list`/`startup:setEnabled`，新面板「启动项」为 BuiltInPanelId 第 7 个）。设计/计划文档：`docs/superpowers/specs/2026-07-29-codemgr-design.md`、`docs/superpowers/specs/2026-07-30-codemgr-desktop-workbench-design.md`、`docs/superpowers/plans/2026-07-30-codemgr-desktop-workbench-remainder.md`、`docs/superpowers/specs/2026-07-31-codemgr-ux-audit.md`、`docs/superpowers/specs/2026-07-31-codemgr-feedback-unification-design.md`、`docs/superpowers/plans/2026-07-31-codemgr-feedback-unification.md`。
- **v2.5**（未发版）：电脑管家首页（Steward Home，spec `docs/superpowers/specs/2026-08-01-codemgr-steward-home-design.md`、plan `docs/superpowers/plans/2026-08-01-codemgr-steward-home.md`）——首页面板（BuiltInPanelId 第 8 个，默认首屏）：状态评估横幅（healthAssess 纯 TS：最差指标主导 + 问题数修正，阈值 70/85、磁盘 20/10 低向、GPU 80/90，≥ 语义，NaN 钳制）+ 五状态卡（响应式 5/2 列，点击直达性能）+ 检测引擎问题清单（IssueDetector 有状态规则：CPU >80% 连续 3 轮 / 进程 cpuPercent≥100 连续 2 轮（经 cpuMap——渲染层 ProcessInfo 无 cpuPercent）/ 内存 3 样本递增 >15% 或 >200MB / 磁盘 ≤10%，占位去重 + 上限 10）+ 快速动作（定位高占用/结束异常/打开性能）。数据自驱：源面板未挂载时首页自采样写共享 store（门控依据是布局树叶子 getPanelLeaves(root)，**不是** visibilityStore.visible——后者不反映挂载）；信息架构：监控组 = 首页/性能/进程/端口雷达/启动项，classic 预设根节点 = home。苹果质感深化（2026-08-06：系统字体栈 SF/Segoe UI Variable/PingFang、侧边栏 macOS 胶囊选择态、材质分层 blur 24px + 侧边栏更透、亮色中性化、苹果动效曲线 0.32,0.72,0,1）。
- 测试：app 646/646 + native 51/51，共 697 PASS。
