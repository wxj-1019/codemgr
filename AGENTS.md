...
- **v2.4**（未发版）：桌面工作台（Apple × Codex Desktop Workbench）Phase 1-5。Phase 1 workspace shell（侧栏+顶栏+panelCatalog）+ 设计系统（语义令牌 surface/content/line/focus/success、lucide facade、UI 原语 Button/IconButton/Badge/StateView/PanelActionBar/PanelAlert、kindStyles）+ 布局引擎（openPanel 幂等/preset/persist v1 迁移/活跃面板协调）。Phase 2 六面板 chrome 统一（去 h1→PanelActionBar、emoji→IconButton+lucide、硬编码色→语义令牌/Badge、ProcessTable colSpan 8→9、PluginPanel bg-base-panel→bg-surface-panel）。Phase 3 响应式 tile（useContainerWidth hook + container query，ProcessPanel 侧栏按 tile 宽度≥720px 而非 matchMedia；Snapshot <480px 切换紧凑顶部控制条；PanelActionBar 窄 tile 自动换行）。Phase 4 portal 浮层（Dialog 组件 createPortal+focus trap+Esc+焦点恢复，ConfirmDialog/DiagnosticPreview/RunProfileEditor 迁移，ContextMenu 加 portal）。Phase 5 桌面交互收口（Workspace shell CSS 合并、标题栏安全区、表格初始 Tab 入口和滚动链、进程详情跨面板聚焦、Snapshot ARIA tabs/ConfirmDialog/reusable StateView）。设计/计划文档：`docs/superpowers/specs/2026-07-30-codemgr-desktop-workbench-design.md`、`docs/superpowers/plans/2026-07-30-codemgr-desktop-workbench-remainder.md`。
- 测试：app 388/388 + native 49/49，共 437 PASS。
