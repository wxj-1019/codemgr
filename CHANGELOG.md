...
### 苹果质感深化（2026-08-06）
- 系统字体栈：macOS 原生 SF / Windows Segoe UI Variable / 中文 PingFang·雅黑（tailwind 配置与 body 双重生效）。
- 侧边栏选择态改为 macOS 风格圆角胶囊高亮 + 加粗（移除竖条指示器），hover 柔和过渡（active 项 hover 保持实色）。
- 材质分层：面板/浮层玻璃提升（blur 24px + saturate 1.2），侧边栏材质更透浮于内容之上；亮色模式底色中性化、玻璃更实。
- 动效曲线统一为苹果标准（150ms cubic-bezier(0.32, 0.72, 0, 1)）。
...
- 首页打磨：自驱采样或面板轮询连续多轮无数据时显示错误态 + 重试按钮；中途数据源失败时保留旧数据并显示「数据陈旧」提示条；CPU/内存卡趋势箭头（↑↓）；磁盘卡显示盘符与剩余；无问题时正向空态。

### 电脑管家首页（Steward Home，2026-08-01）
- 新增「首页」面板（默认首屏）：电脑状态评估横幅（优/良好/需要关注/需要处理，透明规则 + 判定依据文案）+ CPU/内存/磁盘/网络/GPU 五状态卡（响应式 5/2 列，点击直达性能面板）+ 检测引擎问题清单（系统 CPU 持续高 / 单进程 CPU 异常 / 内存增长疑似泄漏 / 磁盘空间低，去重 + 上限 10，严重度排序）+ 快速动作（查看高占用 / 结束异常进程 / 打开性能详情）。
- 问题项与快速动作一键定位到进程面板并选中目标进程（联动复用 openPanel + selectAll 机制）。
- 评估模型 `lib/healthAssess`（最差指标主导 + 问题数修正，阈值与判定依据透明）与检测引擎 `lib/issueDetector`（有状态规则引擎：连续轮触发/占位去重/消除）均为纯 TS。
- 首页数据自驱：源面板（性能/进程）未挂载时首页自行采样（fetchPerf/fetchProcesses/fetchCpu 写入共享 store），挂载时直读 store——任何布局下首页数据可用；窗口不可见时暂停轮询。
- 信息架构：监控组重排为 首页/性能/进程/端口雷达/启动项（启动项由工作流组移入）；classic 布局预设根节点改为首页；侧栏共 8 个内置面板。
