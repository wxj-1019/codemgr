...
> v2.1 修订：补充 §1.4 目标用户画像（3 个人群 + 共同画像 + 排除项），对齐
> v2.1-v2.4 已实现的"AI 开发工作台"特性。
...
> 三个画像回答 §9 风险表头号问题（"开发者真的需要'按项目分组'吗"）。
> 判据：一个功能做不做，先问"三个画像里谁会用、多久用一次"。

**人群总定义**：Windows 上"进程生命周期高频变动"的开发者——dev server 常驻、多项目并行、跑本地 AI 工具链。进程常年稳定的开发者（纯文档办公）不是目标。

#### 人群 1：多项目并行全栈开发者（核心人群）

画像：小陈，前端/全栈，同时维护 2 个前端项目 + 1 个 Node 后端 + 本地 PostgreSQL。任务栏常年 10+ 终端，机器上 7-8 个 node.exe。

| 高频场景 | 频率 | CodeMgr 对应能力 |
|---------|------|-----------------|
| "谁占了我的 3000 端口" | 每天数次 | 端口雷达（首屏差异化） |
| "这个 node 进程属于哪个项目" | 每天 | 按项目分组 |
| 清掉残留 dev 进程 / 一键清空 node | 每天数次 | 批量 kill 同名 + 多选模式 |
| dev server 起不来（端口冲突） | 每周 | 端口冲突高亮 |
| "为什么我的进程读不到这个 env" | 每周 | 按进程查看环境变量 |
| 某进程吃满 CPU/内存排查 | 每周 | 单进程 CPU/内存曲线 |

#### 人群 2：AI 辅助开发 + 本地模型用户（第二圈层，v2.1 起坐实）

画像：小吴，AI agent 重度用户——Claude Code/Codex/Aider 跑长任务，本地 Ollama/LM Studio 挂推理服务，机器常备 GPU。

| 高频场景 | 频率 | CodeMgr 对应能力 |
|---------|------|-----------------|
| "显存被谁占了、还剩多少" | 每天 | GPU/显存监控（无 GPU 降级） |
| agent 跑一天后清理残留进程 | 每天收尾 | 进程快照对比（added 组一键清） |
| 区分 AI 工具进程 vs 业务进程 | 每天 | AI 开发工具默认标签 |
| 一键拉起常用工具链 | 每周 | 运行配置（Run Profiles） |

#### 人群 3：多技术栈"杂食"开发者（泛化外围）

画像：老周，Python/Django + Java + Node 混合维护，一个项目一套工具链，习惯高度自定义。

| 高频场景 | 频率 | CodeMgr 对应能力 |
|---------|------|-----------------|
| 每项目一套进程归组规则 | 一次性 | 自定义标签规则 |
| 桌面塞满面板，按任务切换视图 | 每天 | 多面板工作台 + 布局预设 |
| 同一进程的完整信息（命令行/环境/归属） | 每天 | 进程详情侧栏 |

#### 共同画像

- 个人单机用户：数据全在本地（userData），无云同步/分享场景
- Windows 10/11 x64；机器常驻 200+ 进程（工具链 + IDE + 浏览器）
- 性能敏感（采集 <20ms 红线），但对系统级深挖无兴趣
- kill 类操作追求"一键"，但不接受误杀（保护名单）

#### 明确排除

| 排除人群 | 原因 |
|---------|------|
| SI / Process Explorer 深度用户 | 内核调试、反恶意软件——明确不竞争 |
| 企业/团队用户 | 无协作、无远程管理场景 |
| 非开发者 | 端口雷达、项目分组对非开发者无意义 |
| macOS/Linux 开发者 | 纯 Win32 采集层 |
